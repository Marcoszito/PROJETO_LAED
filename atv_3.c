...
bool colidiu(Vector2 a, float raioA, Vector2 b, float raioB) {
    ...
    return distancia2 <= (somaRaios * somaRaios);
}
...
void desenharItem(Item *item) {
    ...
    switch (item->tipo) {
        case ITEM_ARMA:
            ...
            break;
        case ITEM_POCAO:
            cor = item->dados.pocao.envenenada ? PURPLE : GREEN;
            break;
        case ITEM_ESCUDO:  
            cor = GOLD;
            break;
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawText("Vermelho = arma | Verde = pocao | Roxo = pocao venenosa | Dourado = escudo", 10, 34, 18, GRAY);
